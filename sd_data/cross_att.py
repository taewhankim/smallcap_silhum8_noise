...
class Block(nn.Module):
    ...
    def forward(self, query_embed, kv_embed):
        ...
        if kv_embed is not None:
            kv_embed = self.norm2(kv_embed)
        cross_attn_outputs = self.attn(hidden_states = self.norm1(query_embed), encoder_hidden_states = kv_embed)
...
class CROSS_ATTENTION(nn.Module):
    def __init__(self, pre_config=None, num_heads=12):
        super().__init__()
