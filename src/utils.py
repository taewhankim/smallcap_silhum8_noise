...
class TrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.args.noise_projection:
            ...
            # concate ori image + noise_texted_feature or self att
            encoder_outputs = torch.concat((torch.tensor(encoder_outputs).unsqueeze(0), template_noise_feat.unsqueeze(0)),
                                           dim=0)
            ## ca
            # encoder_outputs = torch.tensor(encoder_outputs).unsqueeze(0)
            # template_noise_feat = template_noise_feat.unsqueeze(0)
        ...
        if self.args.use_ret_img:
            ...
            encoding = {"encoder_outputs": encoder_outputs, # only noise encoder_outputs,
                        "decoder_input_ids": torch.tensor(decoder_input_ids),
                        "labels": torch.tensor(labels),
                        }
            ## cross att
            # encoding = {"encoder_outputs": torch.tensor(encoder_outputs), # encoder_outputs,
            #             "decoder_input_ids": torch.tensor(decoder_input_ids),
            #             "labels": torch.tensor(labels),
