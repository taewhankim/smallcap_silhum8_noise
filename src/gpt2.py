...
class ThisGPT2Attention(GPT2Attention):
    def __init__(self, config, is_cross_attention=False, layer_idx=None):
        ...
        if self.is_cross_attention:
            self.c_attn = Conv1D(int(2 / self.cross_attention_reduce_factor * self.embed_dim),  ###(768,384) (2/4)인 이유는 k,v 2개 이기때문
                                                                                  self.embed_dim) 
    ...
    def forward(
        ...
        if self.reorder_and_upcast_attn:
            ...
            attn_output, attn_weights = self._attn(query, key, value, attention_mask, head_mask) ## 64,12,140,16 , (64,12,140,50)
        
        attn_output = self._merge_heads(attn_output, self.num_heads, int(self.head_dim / self.cross_attention_reduce_factor)) ##(140,192)
