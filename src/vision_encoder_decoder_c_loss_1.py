...
class SmallCap(PreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            text_based_feat = encoder_outputs[:,1,:].clone().detach()

        ## add noise
        encoder_outputs[:,1,:] = clip_noise(template_feat = encoder_outputs[:,1,:],
                                variance=0.016,device=encoder_outputs.device)
