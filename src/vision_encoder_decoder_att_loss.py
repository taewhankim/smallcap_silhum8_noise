...
class SmallCap(PreTrainedModel):
    ...
    def forward(
        ...
        encoder_outputs = torch.sum(encoder_outputs,dim=1).unsqueeze(1)
        ...
        if labels is not None:
            ...
            # ori feat
            encoder_features = encoder_features / encoder_features.norm(dim=-1, keepdim=True)
            cos = torch.nn.CosineSimilarity(dim=1, eps=1e-07)

            # image_features = image_features / image_features.norm(dim=-1, keepdim=True)
            score = cos(encoder_output_ori, encoder_features)
