...
from src.utils import clip_noise
...
class SmallCap(PreTrainedModel):
    ...
    def __init__(
        ...
        ## flat
        self.mlp = MLP(in_features=config.encoder.projection_dim, hidden_features=int(config.encoder.projection_dim*4),act_layer=active,drop=0.1, output_features=self.config.decoder.n_embd)

        scale = self.config.decoder.n_embd ** -0.5
        ## flat
        # self.visual_projection = nn.Parameter(scale * torch.randn(self.config.decoder.n_embd*2, config.encoder.projection_dim))\

        ## vit output 2,512 --> sum --> 1, 512
        # self.visual_projection = nn.Parameter(scale * torch.randn(self.config.decoder.n_embd, config.encoder.projection_dim))
        self.mlp2 = MLP(in_features=config.encoder.projection_dim*2, hidden_features=int(config.encoder.projection_dim*2),act_layer=active,drop=0.1, output_features=config.encoder.projection_dim)
    ...
    def forward(
        ...
        ## 실험 8
        encoder_outputs = encoder_outputs/encoder_outputs.norm(dim=-1, keepdim=True)
        if labels is not None:
            encoder_output_ori = encoder_outputs[:,0,:].clone().detach()
            # text_based_feat = encoder_outputs[:,1,:].clone().detach().unsqueeze(1)

        ## add noise
        ## 수정
        encoder_outputs[:,1,:] = clip_noise(template_feat = encoder_outputs[:,1,:], ### normalize 안하는 상태!!!!
                                variance=0.016,device=encoder_outputs.device) ## 0.016
        ## 수정
        # encoder_outputs = torch.sum(encoder_outputs,dim=1).unsqueeze(1)  ## summation!
        encoder_outputs = encoder_outputs.view(encoder_outputs.shape[0], 1, -1) ## flatten
        encoder_outputs = self.mlp2(encoder_outputs)
        ...
        ## 512
        encoder_features = encoder_hidden_states.clone().detach()
        ...
        if labels is not None:
            ...
            ### 아래 방식은 아닌것같음....
            # # ### 수정 silnum 8
            # image_features = encoder_features.view(encoder_features.shape[0],-1)
            # text_features = text_based_feat.view(text_based_feat.shape[0],-1)
            #
            # image_features = image_features / image_features.norm(dim=-1, keepdim=True)
            # text_features = text_features / text_features.norm(dim=-1, keepdim=True)
            #
            # logits_per_image  = 100 * image_features @ text_features.t()
            # cliploss = self.clip_loss(logits_per_image)
            # loss = loss + 1 * cliploss
    ...
    def clip_loss(self, similarity: torch.Tensor) -> torch.Tensor:
        image_loss = self.contrastive_loss(similarity)
        caption_loss = self.contrastive_loss(similarity.t())
